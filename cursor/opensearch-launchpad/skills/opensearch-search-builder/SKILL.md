...
This skill requires the `opensearch-launchpad` MCP server. In Cursor, add it to `.cursor/mcp.json` (see `cursor/opensearch-launchpad/README.md` in this repo). Example:
...
If `uvx` is not on PATH when Cursor runs, use the bootstrap command in the Cursor plugin README.
